...
xi = 0.7
# times of events:
TT = cumsum(rstable(n,beta, 1, gamma=1, delta=0, pm=1)) / n^(1/beta)
...
where $F_A(\cdot)$ is the CDF of $A:= A(1)$. 
Since 
...
```{r tail-plot, cache=F, echo=F, warning=F, fig.cap="Estimate of tail parameter of Mittag Leffler distribution (y-axis), for observations thresholded at the top k observations (x-axis). The dotted line represents the correct value."}
...
```{r scale-plot, cache=F, echo=F, warning=F, fig.cap="Estimate of scale parameter of Mittag Leffler distribution (y-axis), for observations thresholded at the top k observations (x-axis)."}
...
plot(estimates$topk, estimates$delta * (-log(1-estimates$eps))^(1/estimates$beta), type="l", xlab = "eps", ylab = "delta (beta unknown)", main="scale parameter")
```
