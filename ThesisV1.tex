...
\section{Background}
Before we examine the stochastic process limits that we will utilise in this thesis it will be illustrative to analyse one of the most well known examples of a stochastic process - Brownian Motion. One of the methods used to derive Brownian motion is devised by trying to seek a continuous time representation of the discrete time random walk. Recall that a discrete time random walk is defined in the following manner.\\
\begin{definition}
...
\\
\end{definition}
Now the general method of achieving the continuous limit of a discrete function is to consider the sum $S_n$ as $n\to\infty$ and then rescaling to ensure $S_n$ remains finite. Now by the CLT we have:
...
\noindent The Generalized Central Limit Theorem generalises the Central Limit theorem by dropping the assumption that the variance is finite.
...
\[
	\frac{\sum^n_{i=1}X_i-a_n}{b_n} \overset{d}{\to}Z
\]
