...
Let X$\sim S(\beta,\alpha,\gamma,\delta)$ then the characteristic function $\phi_X(s)=\E[e^{isX}]$ is given by
\[
	\phi_X(s)=\begin{cases}
...
\noindent As there are over half a dozen parameterisations of a stable random variable please note the parameterisation we use is consistent with \cite{MeerschaertStoev08} \& \cite{Samorodnitsky}. Definition \ref{def:stableChar} implies that a stable distribution possesses four different parameters:
\begin{itemize}
...
As we are only interested in heavy tailed
