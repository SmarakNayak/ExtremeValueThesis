...
\documentclass[honours,12pt,twoside, openright]{unswthesis}
\linespread{1.3}
\usepackage{amsfonts}
\usepackage{amssymb}
\usepackage{amsthm}
...
\end{tikzpicture}
\caption{An example CTRM process.}
\label{fig:ctrm}
\end{figure}
...
\begin{theorem}\cite{ColesBook}\label{th:FTG}
...
To sum up, we have now shown that a Mittag-Leffler distribution is appropriate to model the exceedance durations of both the CTRM and its extremal process limit, and we have derived its parameters. 
The next chapter discusses inference on these parameters, as well
as on the parameters of the GEV distribution.
%Thus the process formed by the exceedances and their corresponding durations form a \emph{fractional} Poisson process \cite{Laskin2003}.
...
\chapter{Conclusion}
In this thesis we undertook the task of providing a mathematically well-founded statistical method for predicting extremes of bursty events. %It is understood that a reliable method for predicting such extreme events would have significant impact on our society: it will improve the way emergency response is provided in case of calamities, help better formulate the state financial policies in order to protect customers in case of market meltdown, and help fight spread of epidemics more effectively, to name but a few.
