...
\end{align}
in the Skorokhod space $\D$ with respect to the $J_1$ metric, where $D(t)$ is a $\alpha$-stable subordinator and $S_\Floor{ct}=W_1 + \ldots + W_\Floor{ct}$.
\end{theorem}

Note that the translation scaling constant $a(c)$ is not required when $\alpha\in(0,1)$. We can also adjust $b(c)$ such that the stable subordinator limit is a \emph{unit} stable subordinator with respect to the Laplace transform, defined via 
...
\begin{definition}
	...
	\]
	where F(x) is a GEV distribution. We then say X(t) is an extremal process \textbf{generated} by F(x). Equivalently if $J_i$ are i.i.d. random variables then X(t) is an extremal process if
	\[
