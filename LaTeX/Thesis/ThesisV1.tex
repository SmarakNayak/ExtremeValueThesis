...
where $\lfloor t \rfloor$ denotes the largest integer not larger than $t$.
...
% swapping around alpha and beta confuses me... stability should be alpha
...
In order to introduce the necessary and sufficient conditions for the Generalised Central Theorem to hold we must first define the concept of regular variation.
\begin{definition}\label{def:RV}
	Suppose that $R:[A,\infty)\to(o,\infty)$ is Borel measurable, for some $A>0$. We say that R(x) \textbf{varies regularly} or is \textbf{regularly varying} with index $\rho$, and we write $R\in RV(\rho)$, if
	\begin{align*}
