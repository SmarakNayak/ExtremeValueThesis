...
$[0,\infty)$ and the range is $\R$ as the stochastic process we are concerned with are in this form. Thus we will simply write $\D$ to be the Skorokhod space of $\R$ valued functions defined on $\Rp$.
...
\begin{definition}
	...
	\]
with $S_0=0$. We say that $\{X_n\}$ or $\{S_n\}$ obeys a \textbf{central limit theorem} if there exist a sequence of constants $\{b_n:n\geq 1\}$, a constant $m$ and a nondegenerate random variable S such that there is the following convergence in distribution
...
\end{definition}
In this case our translation scaling constant $m=\mu$ and our space scaling sequence is $\{b_n=\sigma\sqrt{n}:n\geq1 \}$. It is hence obvious that the translation scaling constant and the space scaling sequence adjust for the drift and deviation of the partial sums to ensure convergence.
...
\begin{definition}
	...
	\end{align*}
	under a topology on $\D$.
\end{definition}

\chapter{Approach without scaling limits}
...
In this chapter we will provide an analysis of some well known scaling limit theorems, eventually arriving at a scaling limit for a CTRM process. We require these scaling limits before we can develop our statistical inference methods for a CTRM process.
...
\noindent \textcolor{red}{Maybe move the part below to the simulation section} As we are using the stable distribution to model bursty waiting times we need a completely right skewed distribution with a heavy tail (infinite mean). Thus we consider waiting times drawn from a stable distribution with:
\begin{itemize}
	...
	\item Standard scale parameter $\sigma=1$ and standard location parameter $\mu=0$\\
% "Standard" in terms of Laplace transform means we have to take a scale
% parameter gamma which is not equal to 1, see "simulateML.Rmd"!
\end{itemize}
...
\begin{theorem}\cite{Nolan2015}% CITATION!
% Also, this is not really the GCLT, it's more a possible definition of a 
% sum-stable random variable
...
\noindent In fact if we specify $W_k$ such that $\overline F_W(x)=cx^{-\alpha}$ for some $c>0$, for all $x>c^{1/\alpha}$ and $0<\alpha<1$. Then $\overline F_W(x)$ is clearly regularly varying with index $-\alpha$ and by \cite[Th~3.39]{MeerschaertSikorskii2012} we have the more specific result
...
\begin{align}\label{eq:partialParetoSum}
	n^{-1/\alpha}\sum\limits^\Floor{nt}_{j=1} W_j \cd Z(t)
\end{align}
for all $t>0$, where
\begin{align}
...
\]
\end{proof}
Furthermore if we assume The i.i.d.\ nature of the $W_k$ allows for a more general, functional limit
...
\end{align}
where convergence holds on the stochastic process level, i.e.\ in the sense
